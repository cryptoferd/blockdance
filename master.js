/* master.js — Bitcoin Atom renderer (p5 instance-mode, no network) */
/* global p5 */
(function () {
  // ---------- small utils (deterministic) ----------
  ...
  const paletteFromHeight=(height)=>{const rng=XS32(height), golden=137.50776405003785; let baseHue=mod((height*golden)+rng()*45,360);
  ...
  function buildAtomFromTraits(b){
    ...
    const halving=height>0 && (height%210000)===0;

    let colors=(function(height){const rng=XS32(height), golden=137.50776405003785; let baseHue=mod((height*golden)+rng()*45,360);
      let hP=baseHue, hN=mod(baseHue+(90+rng()*120),360), hE=mod(baseHue+(180+rng()*120),360), hS=mod(baseHue+(30+rng()*120),360);
      let sP=0.82+0.18*rng(), lP=0.48+0.12*rng(); let sN=0.72+0.25*rng(), lN=0.52+0.12*rng(); let sE=0.92+0.08*rng(), lE=0.58+0.12*rng(); let sS=0.82+0.18*rng(), lS=0.56+0.12*rng();
      return{ proton:hslToRgb(hP/360,sP,lP), neutron:hslToRgb(hN/360,sN,lN), electron:hslToRgb(hE/360,sE,lE), shell:hslToRgb(hS/360,sS,lS) };})(height);
    ...
    const weightOrSize=weight||size*4, eSizeScale=(((weightOrSize-2.8e6)/(4.0e6-2.8e6))*(1.50-0.90)+0.90);
    const dens=clamp(weight/Math.max(1,size*4),0.6,1.4), nonceNorm=(nonce>>>0)/4294967295;
    let nucleusScale=((1.10-0.65)*nonceNorm+0.65)*dens*(1.0/Math.sqrt(luck));
    const spreadTx=clamp((txCount-1)/(6000-1)*(1.35-0.9)+0.9,0.9,1.35), spreadVer=1.0+((version&0xF)-7.5)*0.01;
    const orbitSpread=spreadTx*spreadVer, radii=shells.map((_,i)=>(NUCLEUS_R0*1.3 + SHELL_STEP*(i+1))*orbitSpread);
    ...
    const hashLast=lastByte(hash), ringThickBase=0.8+(hashLast/255)*2.0, ringThick=ringThickBase*(((version>>0)&1)?1.8:1.0);
    const ringAlpha=clamp(140+(luck-1.0)*120,80,255), hueShiftDeg=(hashLast/255-0.5)*36, ringColor=shiftHue(colors.shell,hueShiftDeg);
    ...
    const timeSkew=clamp((timestamp-mediantime)/600,-1,1), wobbleAmp=((1.03-0.97)*(timeSkew+1)/2+0.97), wobbleFreq=0.5+Math.abs(timeSkew)*0.8;
    ...
    return {
      block:b, height, Z, N, electrons, shells, radii, speeds, electronAngles, nucleusPoints, colors,
  ...
  function makeSketch(traits){
    ...
    const stars = []; const STAR_N= atom.style.halving ? 0 : 300;

    const sketch = (p)=>{
      const VERTs = VERT, FRAGs = FRAG;
      const ELEC_R=ELECTRON_R, NUCL_R=NUCLEON_R;

      p.setup = function(){
        p.createCanvas(p.windowWidth, p.windowHeight, p.WEBGL);
        p.setAttributes('antialias', true);
        orbShader = p.createShader(VERTs, FRAGs);
        for(let i=0;i<STAR_N;i++){ stars.push({x:p.random(-2800,2800), y:p.random(-2800,2800), z:p.random(-2800,2800), s:p.random(0.4,1.0)}); }
      };
      ...
      function drawOrbFixed(colorA, colorB, radius, t, metal, rough){
        ...
        if (SHADER_OK){
          ...
          }catch(e){ SHADER_OK=false; p.resetShader(); }
        }
      ...
      function drawNucleusGlowOverlay(){
        ...
        p.blendMode(p.ADD);
        const baseR = 28 * (atom.style.nucleusScale || 1.0);
      ...
      function drawNormal(t){
        if (STAR_N){ p.push(); p.noStroke(); p.ambientMaterial(200); for(const s of stars){ p.push(); p.translate(s.x,s.y,s.z); p.sphere(s.s); p.pop(); } p.pop(); }
        ...
        for(const q of atom.nucleusPoints){
          ...
          drawOrb(q.isProton? atom.colors.proton : atom.colors.neutron,
                  q.isProton? atom.colors.neutron : atom.colors.proton, 2.7, t);
          p.pop();
        }
        p.pop();
        ...
        for(let i=0;i<atom.shells.length;i++){
          ...
          p.push(); p.noFill(); p.stroke(...atom.style.ringColor, atom.style.ringAlpha); p.strokeWeight(atom.style.ringThick);
        ...
        for(let i=0;i<atom.shells.length;i++){
          ...
          for(let j=0;j<atom.shells[i];j++){
            ...
            p.push(); p.translate(ex,ey,ez); drawOrb(eCol, atom.style.ringColor, 3.6 * atom.style.eSizeScale, t); p.pop();
          }
        }
      }

      function drawHalving(t){
        p.push();
        const breath = 1.0 + (Math.sin(t * atom.style.wobbleFreq*2.0)*0.02) * (atom.style.wobbleAmp-1.0);
        p.scale(breath, 1.0, 1.0);
        for(const q of atom.nucleusPoints){
          p.push(); p.translate(q.x,q.y,q.z); drawOrbFixed(atom.style.colors.gold, atom.style.colors.gold, 2.7, t, atom.style.metalnessGold, atom.style.roughGold); p.pop();
        }
        p.pop();

        for(let i=0;i<atom.shells.length;i++){
          const r=atom.radii[i];
          p.push(); p.noFill(); p.stroke(255,136,0,160); p.strokeWeight(atom.style.ringThick*1.1);
          p.beginShape(); const steps=160; for(let k=0;k<steps;k++){ const ang=(k/steps)*Math.PI*2; p.vertex(r*Math.cos(ang), r*Math.sin(ang), 0); }
          p.endShape(p.CLOSE); p.pop();
        }
        ...
        for(let i=0;i<atom.shells.length;i++){
          ...
          for(let j=0;j<atom.shells[i];j++){
            const ang=atom.electronAngles[i][j], tilt=atom.shellTilt[i]+atom.eTiltJitter[i][j];
            const ex=(r*aMul)*Math.cos(ang), ey=(r*bMul)*Math.sin(ang)*Math.cos(tilt), ez=(r*bMul)*Math.sin(ang)*Math.sin(tilt);
            const phase=(i*0.7+j*1.13), pulse=1.0+0.22*Math.sin(t*spd+phase), rad=3.6*atom.style.eSizeScale*pulse;
            p.push(); p.translate(ex,ey,ez); drawOrbFixed(atom.style.colors.silver, atom.style.colors.silver, rad, t, atom.style.metalnessSilver, atom.style.roughSilver); p.pop();
          }
        }

        const gl = p._renderer.GL;
        p.push(); p.resetShader(); p.noStroke(); gl.disable(gl.DEPTH_TEST); gl.depthMask(false); p.blendMode(p.ADD);
        const baseR = 28 * (atom.style.nucleusScale || 1.0), glowR = baseR*2.8;
        for(let i=0;i<5;i++){ const u=i/4, al=Math.round(28*(1.0*u<=1.0?1.0-u:0)); p.fill(255,160,0,al); p.push(); p.sphere(glowR*(1.0+u*0.55)); p.pop(); }
        p.blendMode(p.BLEND); gl.depthMask(true); gl.enable(gl.DEPTH_TEST); p.pop();
      }

      p.draw = function(){
        if (!p.mouseIsPressed) p.orbitControl(1,1,1.5);
        ...
        if (atom.style.halving) drawHalving(t); else drawNormal(t);
  ...
  window.__start = function(TRAITS){
    ...
    wrap.style.position='fixed'; wrap.style.inset='0'; document.body.appendChild(wrap);
